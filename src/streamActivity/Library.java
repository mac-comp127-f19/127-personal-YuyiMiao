...
import marsh.f19.inheritance2.B;
...
//        return result;
        ...
        return books.stream()
                ...
                .map(temp -> temp.getTitle() + " by " + temp.getAuthor())
                .collect(toList());
    }
...
//            .collect(toList());
        ...
        Comparator<Book> compB =
                (Book b1, Book b2) -> Integer.compare(b1.getPageCount(), b2.getPageCount());
        Collections.sort(copyBooks,compB);
        List<Book> result = new ArrayList<>();
        int count = 0;
        for(Book d : copyBooks){
            if(count == n){
                break;
            }
            result.add(d);
...
//            .orElse(null);
        ...
        if (nullExist == false) {
            Comparator<Book> compB =
                    (Book b1, Book b2) -> Integer.compare(b1.getTitle().length(), b2.getTitle().length());
            Collections.sort(copyBooks,compB);
        }
        return copyBooks.get(0);
    }
